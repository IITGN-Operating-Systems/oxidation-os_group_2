...
//         //     Err(io::Error::new(io::ErrorKind::InvalidData, expected))
//         // }
//         const TIMEOUT: u64 = 5; // Timeout after 5 attempts
//         let mut attempts = 0;
...
//             attempts += 1;
//             std::thread::sleep(std::time::Duration::from_millis(50)); // Sleep for a small period before retry
//         }
...
//                 return Ok(0);
//             }
//             CAN => return ioerr!(ConnectionAborted, "received CAN"),
//             _ => return ioerr!(InvalidData, "expected SOH or EOT"),
//         }
...
//         // unimplemented!()
//         if self.packet == 0 {
//             self.inner.write_all(&[EOT])?;
//             if self.read_byte(false)? != NAK {
//                 return Err(io::Error::new(io::ErrorKind::InvalidData, "expected NAK after first EOT"));
//             }
//             self.inner.write_all(&[EOT])?;
//             return Ok(0);
//         }
...
//             for _ in 0..5 {  // Try 5 times to get NAK
//                 if let Ok(byte) = self.read_byte(false) {
//                     if byte == NAK {
//                         break;
//                     }
//                 }
//                 std::thread::sleep(std::time::Duration::from_millis(10)); // Small delay before retry
//             }
//             self.write_byte(EOT)?;
...
//         if !self.started {
//             self.expect_byte(NAK, "expected initial NAK")?;
//             self.started = true;
//             (self.progress)(Progress::Started);
//         }
...
//                 NAK => continue,
//                 CAN => return ioerr!(ConnectionAborted, "received CAN"),
//                 _ => return ioerr!(InvalidData, "unexpected response"),
//             }
//         }

//         self.write_byte(CAN)?;
//         ioerr!(BrokenPipe, "failed to send packet")
//     }
